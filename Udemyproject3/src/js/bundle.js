...
function calc() {
  ...
  if (localStorage.getItem('sex')) {
    //проверка естьли что-то в localStorage
  ...
  function initLocalSettings(selector, activeClass) {
    // когда зах на стр чт подтягивались данные из localStorage
  ...
  function getDynamicInformation(selector) {
    ...
    input.addEventListener('input', () => {
      ...
      switch (input.getAttribute('id')) {
        // проверяем какой именно инпут нажат
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  document.addEventListener('keydown', e => {
    // чт закрывалось при Esc
    if (e.code === 'Escape' && modal.classList.contains('show')) {
      //если наж Esc и открыто мод окно
      closeModal(modalSelector);
    }
  });
...
function slider(_ref) {
  ...
  slides.forEach(slide => {
    //т к слайды м б разной шир--устан для них одну ширину
  ...
  next.addEventListener('click', () => {
    if (offset == deleteNotDigits(width) * (slides.length - 1)) {
      //slice(0, width.length - 2)заменила на replace(/\D/g, '')
    ...
    if (slideIndex == slides.length) {
      //если мы на последнем-то перемещ-ся на первый
      slideIndex = 1;
    } else {
      slideIndex++;
    }
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  function showTabContent() {
    ...
    //показ-ем i-й эл с  .tabheader__item_active
  ...
  tabsParent.addEventListener('click', event => {
    ...
    if (target && target.classList.contains(tabsSelector.slice(1))) {
      // тк нам надо передавать 'tabheader__item'без точки
...
function timer(id, deadline) {
  ...
  function getTimeRemaining(endtime) {
    ...
    if (t <= 0) {
      //если время истекло то пишем нули.Тут можно вставить блок с надписью -акция оконч ,другое
      days = 0;
      hours = 0;
      minutes = 0;
      seconds = 0;
  ...
  function setClock(selector, endtime) {
    ...
    function updateClock() {
      //будет обновлять таймер каждую секунду
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__["default"])('.timer', '2023-05-11');
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).ready(function () {
    console.log('jljhj');
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('.example-jquery .list-item:first').hover(function () {
      jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).toggleClass('jquery-active');
    });
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('.example-jquery .list-item:eq(2)').on('click', function () {
      jquery__WEBPACK_IMPORTED_MODULE_0___default()('.example-jquery .image:even').fadeToggle('slow');
    });
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('.example-jquery .list-item:eq(4)').on('click', function () {
